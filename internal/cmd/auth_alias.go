...
func (c *AuthAliasListCmd) Run(ctx context.Context) error {
	...
	_, _ = fmt.Fprintln(w, "ALIAS\tEMAIL")
...
func (c *AuthAliasSetCmd) Run(ctx context.Context) error {
	...
	if alias == "" {
		return usagef("empty alias")
	}

	if strings.Contains(alias, "@") {
		return usagef("alias must not contain '@'")
	}

	if shouldAutoSelectAccount(alias) {
		return usagef("alias name is reserved")
	}
